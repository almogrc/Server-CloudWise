...
namespace Server_cloudata.Services.Collector
{
    public class ThresholdsCollector
    {
        ...
        public async Task<Metric> AddThresholsToMetric(Metric metric, string machineAddress)
        {
            try
            {
                ...
                if (virtualMachine.Thresholds.ContainsKey(metric.DataType))
                {
                    ...
                    {
                        float totalRam = (await _collector.GetData(eNodeExporterData.Ram.ToString(), DateTime.UtcNow.AddMinutes(-1), DateTime.UtcNow, machineAddress)).DataPoints[0].Value;
